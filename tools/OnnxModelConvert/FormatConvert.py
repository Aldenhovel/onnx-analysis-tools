...
from tools.OnnxModel import save_onnx
...
def cvt_cfg2onnx(cfg_file: str, weights_file: str, output_file: str, neck: str="PAN", strides: List = []):
    ...
    if len(strides) == 0:
        ...
        for conv_layer, num_anchor in zip(conv_layers, num_anchors):
            output_tensor_dims[conv_layer] = [(classes + 5) * num_anchor, 0, 0]
        ...
        yolo_model_def = onnx.shape_inference.infer_shapes(yolo_model_def)
        output_tensor_dims = OrderedDict()
        for conv_layer in conv_layers:
            conv_layer_shape = _get_tensor_shape(yolo_model_def, conv_layer)
            output_tensor_dims[conv_layer] = [(classes + 5) * num_anchor] + conv_layer_shape[-2:]
    ...
    onnx.checker.check_model(yolo_model_def)
    yolo_model_def = onnx.shape_inference.infer_shapes(yolo_model_def)
    yolo_model_def = OnnxGraph.remove_initializer_from_input(yolo_model_def)

    return yolo_model_def


def _get_tensor_shape(model: onnx.ModelProto, tensor_name: str) -> List:
    graph = model.graph
...
if __name__ == '__main__':
    ...
    cfg_file = "../../models/yolov4/yolov4.cfg"
    weights_file = "../../models/yolov4/yolov4.weights"
    output_file = "../../models/yolov4/yolov4-cvt-xx.onnx"
    neck = "PAN"
    strides = []
    # strides = [] ==> auto mode, slower
    # or set strides = [8, 16, 32] to use stride mode, faster

    import time

    t1 = time.time()
    model = cvt_cfg2onnx(cfg_file=cfg_file, weights_file=weights_file, output_file=output_file, neck=neck, strides=strides)
    print(f"time cost: {time.time() - t1} s.")

    save_onnx(model, output_file)
